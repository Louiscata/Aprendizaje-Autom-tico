...
print("=" * 66)
print("  ADESTRAMENTO DO META-MODELO (CV temporal + final)")
print("=" * 66)
...
# ── 8a. CV temporal sobre as meta-features ────────────────────────────────────
META_CV_FOLDS = 5
META_CV_GAP   = 100   # gap máis pequeno: o meta-conxunto é xa reducido

tscv_meta = TimeSeriesSplit(n_splits=META_CV_FOLDS, gap=META_CV_GAP)
meta_f1s  = []

# Reordenar polo índice temporal para garantir a honestidade do split
order_meta   = dates_meta.argsort().values
X_meta_ord   = X_meta.iloc[order_meta].reset_index(drop=True)
y_meta_ord   = y_meta.iloc[order_meta].reset_index(drop=True)
dates_meta_ord = dates_meta.iloc[order_meta].reset_index(drop=True)

print(f"  CV temporal do meta-modelo  (folds={META_CV_FOLDS}, gap={META_CV_GAP}):")
print(f"  {'Fold':<6} {'train_size':>10} {'val_size':>9} {'val_desde':<12} {'F1-macro':>9}")
print(f"  {'-'*52}")

for fold_m, (idx_tr_m, idx_val_m) in enumerate(
        tscv_meta.split(X_meta_ord), 1):

    d_max_m = dates_meta_ord.iloc[idx_tr_m].max().date()
    d_min_m = dates_meta_ord.iloc[idx_val_m].min().date()

    Xtr_m  = X_meta_ord.iloc[idx_tr_m]
    ytr_m  = y_meta_ord.iloc[idx_tr_m]
    Xval_m = X_meta_ord.iloc[idx_val_m]
    yval_m = y_meta_ord.iloc[idx_val_m]

    cb_cv = CatBoostClassifier(**{**META_PARAMS, 'verbose': 0})
    cb_cv.fit(
        Pool(Xtr_m,  label=ytr_m,  cat_features=cat_idx_meta),
        eval_set=Pool(Xval_m, label=yval_m, cat_features=cat_idx_meta),
    )
    preds_val_m = cb_cv.predict(
        Pool(Xval_m, cat_features=cat_idx_meta)
    ).ravel().astype(int)

    f1_m = f1_score(yval_m, preds_val_m, average='macro')
    meta_f1s.append(f1_m)
    print(f"  {fold_m:<6} {len(idx_tr_m):>10} {len(idx_val_m):>9} "
          f"{str(d_min_m):<12} {f1_m:>9.5f}")

print(f"  {'-'*52}")
print(f"  {'Media':>6} {'':>10} {'':>9} {'':>12} "
      f"{np.mean(meta_f1s):>9.5f}  ±{np.std(meta_f1s):.5f}\n")

# ── 8b. Adestramento final sobre todo o meta-conxunto ─────────────────────────
print("  Adestramento final do meta-modelo (todo o OOF)...")
...
nome_arquivo = f'./resultados/28-04-2026_3_catboost-stacking_final_top{len(BEST_FEATS)}.csv'
